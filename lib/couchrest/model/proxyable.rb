  ...
  module Model
    ...
    module Proxyable
      ...
      def proxy_database(assoc_name, suffix = nil)
        ...
        @proxy_databases ||= {}
        @proxy_databases[assoc_name.to_sym] ||= self.class.prepare_database([self.send(self.class.proxy_database_method), suffix].compact.reject(&:blank?).join(self.class.connection[:join]), true)
      end
